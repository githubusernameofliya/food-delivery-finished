...
BOT_TOKEN = "7183153809:AAGaypTvt6i7NOD9gSkwv7lWm9axBiSI210"
PAYMENTS_PROVIDER_TOKEN = "371317599:TEST:1716378841081"
...
# Setup prices
prices = [
    types.LabeledPrice(label='Working Time Machine', amount=5750000),  # 57.50 UZS
    types.LabeledPrice(label='Gift wrapping', amount=500000),         # 5.00 UZS
]

# Setup shipping options
shipping_options = [
    types.ShippingOption(id='instant', title='WorldWide Teleporter').add(types.LabeledPrice('Teleporter', 100000)),  # 1.00 UZS
    types.ShippingOption(id='pickup', title='Local pickup').add(types.LabeledPrice('Pickup', 30000)),                # 0.30 UZS
]
...
async def cmd_start(message: types.Message):
    await bot.send_message(
        message.chat.id,
        "Hello, I'm the demo merchant bot. I can sell you a Time Machine. Use /buy to order one, /terms for Terms and Conditions"
    )
...
async def cmd_buy(message: types.Message):
    await bot.send_message(
        message.chat.id,
        "Real cards won't work with me, no money will be debited from your account. Use this test card number to pay for your Time Machine: `4242 4242 4242 4242`\n\nThis is your demo invoice:",
        parse_mode='Markdown'
    )
...
async def got_payment(message: types.Message):
    await bot.send_message(
        message.chat.id,
        'Hoooooray! Thanks for the payment! We will proceed with your order for `{} {}` as fast as possible! Stay in touch.\n\nUse /buy again to get a Time Machine for your friend!'.format(
